...
class ForumBox extends Component {
  ...
  handleCreateForum () {
    ...
    const { newForumTitle, newForumSlug } = this.state;
    ...
    if (!convertedTitle) {
      return this.setState({ errorMsg: 'Please provide a valid Forum Title.' });
    }
    if (!convertedSlug) {
      return this.setState({
        errorMsg:
          'Please provide a valid Forum Slug. Slug can only contain small case alphabets and underscore.',
      });
    }

    if (convertedTitle && convertedSlug) {
      this.props.createAction({ title: convertedTitle, slug: convertedSlug });
    }
  }

  render () {
    const { forums, creatingForum, deleteAction, deletingForum } = this.props;

    const { newForumTitle, newForumSlug, errorMsg } = this.state;
